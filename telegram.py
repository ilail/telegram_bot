...
def get_schedule(web_page, day):
    ...
    return times_list, locations_list, lessons_list, hall_list, day_name
...
@bot.message_handler(commands=list(config.day_in_week.keys()))
def get_exact_day(message):
    day, week, group = message.text.split()
    if day in config.day_in_week.keys():
        day_n = config.day_in_week.get(day)
    if week in config.week_list.values(): 
        week_n = week
    elif week in config.week_list.keys():
        week_n = config.week_list.get(week)
    web_page = get_page(group, week_n)
    times_list, locations_list, lessons_list, hall_list = get_schedule(web_page, day)
    resp = ''
    for time, cab, location, lesson in zip(times_list, locations_list, lessons_list, hall_list):
